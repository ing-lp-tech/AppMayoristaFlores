...
import { Package, Search, ChevronDown, ChevronUp, AlertCircle, Clock } from 'lucide-react';
import { StockMovementsList } from '../../components/admin/Products/StockMovementsList';
    ...
    return (
        <div className="p-6 max-w-7xl mx-auto">
            ...
            <div className="flex border-b border-gray-200 mb-6">
                <button
                    className={`px-6 py-3 font-medium text-sm flex items-center gap-2 border-b-2 transition-colors ${activeTab === 'stock'
                ...
                <button
                    className={`px-6 py-3 font-medium text-sm flex items-center gap-2 border-b-2 transition-colors ${activeTab === 'historial'
                            ? 'border-blue-600 text-blue-600'
                            : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'
                        }`}
                                                                        ...
                                                                        {product.talles
                                                                            ...
                                                                            .map((talle: any) => (
                                                                                <div key={talle.id} className="bg-white p-3 rounded-lg border border-gray-200 shadow-sm text-center">
                                                                                    ...
                                                                                    <div className="text-[10px] text-gray-400">unids. total</div>

                                                                                    {talle.stock_por_color && Object.keys(talle.stock_por_color).length > 0 && (
                                                                                        <div className="mt-2 pt-2 border-t border-gray-100 flex flex-col gap-1 text-left">
                                                                                            {Object.entries(talle.stock_por_color).map(([color, count]) => (
                                                                                                <div key={color} className="flex justify-between items-center text-[10px]">
                                                                                                    <div className="flex items-center gap-1">
                                                                                                        <div className="w-2 h-2 rounded-full border border-gray-200" style={{ backgroundColor: product.colores?.find((c: any) => c.nombre === color)?.hex || '#ccc' }}></div>
                                                                                                        <span className="text-gray-600 truncate max-w-[50px] overflow-hidden" title={color}>{color}</span>
                                                                                                    </div>
                                                                                                    <span className={`font-bold ${Number(count) === 0 ? 'text-red-500' : 'text-gray-800'}`}>{String(count)}</span>
                                                                                                </div>
                                                                                            ))}
                                                                                        </div>
                                                                                    )}
